...
def predict(inputText):
    ...
    pemopath=''
    sentence = []
    sentence.append(inputText)
    sequence= tokenizer.texts_to_sequences(sentence)
    padded = pad_sequences(sequence,maxlen=100,padding=padding_type,truncating=trunc_type)
    padded1 = np.array(padded)
    prediction = np.argmax(model.predict(padded1),axis=1)
    for key,value in emo_code_url.items():
        if value[0] == prediction:
            pemo = key
            pemopath = value[1]
    return pemo, pemopath 
